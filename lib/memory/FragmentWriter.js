...
export class FragmentWriter {
  ...
  async deleteMany(ids, agentId = "default", keyId = null) {
    ...
    const args = [ids];
    if (keyId) {
      sql += ` AND (key_id IS NULL OR key_id = $2)`;
      args.push(keyId);
    }
  ...
  async patchAssertion(id, assertionStatus, keyId = null) {
    const agentId = keyId != null ? "default" : "system";
   ...
   * @returns {Promise<boolean>} 업데이트 성공 여부
   */
  async updateCaseId(id, caseId) {
    const result = await queryWithAgentVector("system",
      `UPDATE ${SCHEMA}.fragments SET case_id = $2 WHERE id = $1 AND valid_to IS NULL`,
      [id, caseId],
      "write"
    );
    return (result.rowCount || 0) > 0;
