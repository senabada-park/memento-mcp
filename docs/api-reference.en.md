...
The legacy top-level `_searchEventId` / `_memento_hint` / `_suggestion` fields remain for backward compatibility via mirror in v3.0.0 but are **DEPRECATED — scheduled for removal in v3.1.0**. Clients should migrate to the `_meta` path.

`_suggestion` (v2.9.0+, DEPRECATED — removal in v3.1.0, use `_meta.suggestion`): A hint object generated by the RecallSuggestionEngine based on analysis of the current search pattern. `null` when no issue is detected.
