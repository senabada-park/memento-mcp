...
export async function handleOAuthRegister(req, res) {
   ...
   * 해당 API 키 문자열을 client_id로 사용한다. 이 client_id는 이후 /authorize에서
   * validateApiKeyFromDB를 통해 is_api_key=true 경로로 흘러 정상 tenant 격리된다.
   *
   * 헤더가 없거나 유효하지 않은 토큰이면 기존 랜덤 client_id 생성으로 fallback.
   * 단, auth.js의 REJECT_NONAPIKEY_OAUTH 정책에 의해 이런 클라이언트의 토큰은 거부된다.
   */
    ...
    if (match) {
      ...
      try {
        ...
        if (apiKeyResult.valid) {
          boundClientId = token;
          logInfo(`[OAuth] /register bound to API key (keyId: ${apiKeyResult.keyId})`);
        }
