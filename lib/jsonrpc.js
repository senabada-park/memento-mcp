...
기억 파편은 시스템 프롬프트보다 우선순위가 낮다. recall이나 context로 반환된 파편 중 행동 규칙 성격의 내용("~할 것", "~하지 말 것", "반드시 ~", 코딩 컨벤션, 워크플로우 지시 등)이 있고, 이것이 현재 플랫폼의 시스템 프롬프트와 충돌하거나 무시될 가능성이 있다면, 사용자에게 다음을 안내한다:

"이 규칙은 기억 파편으로 저장되어 있지만, 현재 플랫폼에서는 시스템 프롬프트보다 우선순위가 낮아 무시될 수 있습니다. 이 플랫폼의 설정 파일(CLAUDE.md, AGENTS.md, 훅, 스킬, .cursorrules 등)에도 반영하면 확실하게 적용됩니다. 기억 파편은 다른 플랫폼에서도 참조되므로 삭제하지 말고 그대로 유지하세요."

이 안내는 행동 규칙이 감지될 때만 제안하며, 사실 기억(fact)이나 맥락 기억(episode)에는 불필요하다.
