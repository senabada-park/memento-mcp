...
export const OAUTH_ACCESS_TTL_SECONDS  = Number(process.env.OAUTH_ACCESS_TTL_SECONDS || 3600);
export const OAUTH_REFRESH_TTL_SECONDS = Number(process.env.OAUTH_REFRESH_TTL_SECONDS || 604800);
/** @deprecated 하위 호환 alias — OAUTH_ACCESS_TTL_SECONDS 사용 권장 */
export const OAUTH_TOKEN_TTL_SECONDS   = OAUTH_ACCESS_TTL_SECONDS;
...
/**
 * OAuth trusted origins — redirect_uri 정확 일치 허용 origin 목록.
 *
 * 의미 변경 (silent consent 제거 후):
 *   - 이전: 자동 승인(silent consent) 대상 도메인
 *   - 이후: redirect_uri origin 단위 허용 목록. consent 화면은 항상 경유.
 *
 * 이전 기본값(claude.ai/chatgpt.com 등)은 silent consent 제거로 더 이상
 * 자동 승인 용도로 사용되지 않는다. 운영자가 명시적으로 환경변수에 설정해야 한다.
 *   예: OAUTH_TRUSTED_ORIGINS=https://claude.ai,https://chatgpt.com
 *
 * 기본값 empty: 환경변수 미설정 시 origin 허용 없음 (localhost는 별도 허용).
 */
export const OAUTH_TRUSTED_ORIGINS = String(process.env.OAUTH_TRUSTED_ORIGINS || "")
  .split(",")
  .map(v => v.trim())
  .filter(Boolean);
